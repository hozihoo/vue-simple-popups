...
<template>
  <TabsRoot
    v-model="currentTab"
    class="bg-background-deep border-t border-x-none border-b-none border-solid border-border overflow-hidden rounded-b-xl"
    @update:model-value="open = true"
  >
    <div
      class="border-b border-x-none border-t-none border-solid border-border bg-background flex pr-2"
    >
        ...
        <TabsList class="relative flex">
          <template v-if="open">
            ...
            <TabsTrigger
              ...
              :key="index"
              :value="tab.label"
              class="box-border text-foreground px-4 py-3 data-[state=active]:text-[var(--vp-c-indigo-1)]"
              tabindex="-1"
            >
              {{ tab.label }}
            </TabsTrigger>
